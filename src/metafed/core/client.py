...
class Client:
    ...
    def get_model_buffers(self) -> Dict[str, torch.Tensor]:
        """
        Get current model buffers (e.g. BatchNorm running_mean, running_var).
        Required so the server can aggregate them; otherwise eval accuracy stays
        at random because the global model's BN stats are never updated.
        
        Returns:
            Dictionary of buffer names to cloned tensors
        """
        return {name: buf.clone() for name, buf in self.model.named_buffers()}
